...
@mock.patch(
    "scigateway_auth.src.admin.MAINTENANCE_CONFIG_PATH", MAINTENANCE_CONFIG_PATH
)
class TestMaintenanceMode(TestCase):
    ...
    @mock.patch(
        "builtins.open", mock.mock_open(read_data=json.dumps(MAINTENANCE_STATE))
    )
    ...
    def test_set_state_valid_non_admin_token(self):
        result = self.maintenance_mode.set_state(
            VALID_NON_ADMIN_ACCESS_TOKEN, MAINTENANCE_STATE
        )
        self.assertEqual(result, ("Unauthorized", 403))

    def test_set_state_token_without_admin_info(self):
        result = self.maintenance_mode.set_state(
            ACCESS_TOKEN_WITHOUT_ADMIN_INFO, MAINTENANCE_CONFIG_PATH
        )
        self.assertEqual(result, ("Unauthorized", 403))
